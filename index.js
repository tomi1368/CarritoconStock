...
app.post("/api/pay", async (req, res) => {
  let error = false;
  const responsa = req.body;
  const productCopy = products.map((product) => ({ ...product }));
  let preference = {
    items: [],
    back_urls: {
  ...
  responsa.forEach((p) => {
    const product = productCopy.find((e) => e.id == p.id);
    if (product.stock > 0) {
